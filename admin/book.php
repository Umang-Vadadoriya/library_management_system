...
if (isset($_POST["edit_book"])) {
	...
	if ($error == '') {
		$formdata['book_id'] = $_POST["book_id"];
		$query = "
		UPDATE lms_book 
        SET book_category = '" . $formdata['book_category'] . "', 
        ...
        WHERE book_id = " . $formdata['book_id'];
					...
					<div class="col-md-6">
						<div class="mb-3">
							...
							<select name="book_location_rack" id="book_location_rack" class="form-control">
								<?php echo fill_location_rack($conn); ?>
							</select>
						</div>
					</div>
				</div>
				...
				<div class="row">
					<div class="col-md-6">
						<div class="mb-3">
							<label class="form-label">Book Name</label>
							<input type="text" name="book_name" id="book_name" class="form-control"
								value="<?php echo $book_row['book_name']; ?>" />
						</div>
					</div>
					<div class="col-md-6">
						<div class="mb-3">
				...
				<div class="row">
					<div class="col-md-6">
						<div class="mb-3">
							<label class="form-label">Book ISBN Number</label>
							<input type="text" name="book_isbn_number" id="book_isbn_number" class="form-control"
								value="<?php echo $book_row['book_isbn_number']; ?>" />
						</div>
					</div>
					<div class="col-md-6">
						<div class="mb-3">
							...
							<input type="number" name="book_no_of_copy" id="book_no_of_copy" class="form-control"
								step="1" value="<?php echo $book_row['book_no_of_copy']; ?>" />
						</div>
					</div>
				</div>
				<div class="mt-4 mb-3 text-center">
						...
						if (mysqli_num_rows($result_show) > 0) {
							foreach ($result_show as $row) {
								...
								if ($row['book_status'] == 'Enable') {
									...
									$book_status_btn = '<button type="button" name="switch_button" class="btn btn-warning btn-sm mb-1" onclick="switch_data(`' . $row["book_id"] . '`, `' . $row["book_status"] . '`)">Disable</button>';
								} else {
									$book_status = '<div class="badge bg-danger">Disable</div>';
									$book_status_btn = '<button type="button" name="switch_button" class="btn btn-success btn-sm mb-1" onclick="switch_data(`' . $row["book_id"] . '`, `' . $row["book_status"] . '`)">Enable</button>';
								}
								echo '
        				<tr>
        					<td>' . $row["book_name"] . '</td>
        					...
        					<td>
        						<a href="book.php?action=edit&code=' . $row["book_id"] . '" class="btn btn-sm btn-primary mb-1">Edit</a>
								' . $book_status_btn . '
								<a class="btn btn-sm btn-danger" onclick="delete_data(`' . $row["book_id"] . '`)">Delete</a>
        					</td>
        				</tr>
        				';
							}
	...
	<script>
		function switch_data(code, status) {
			...
			if (confirm("Are you sure you want to " + new_status + " this Category?")) {
				window.location.href = "book.php?action=switch&code=" + code + "&status=" + new_status + "";
			}
		}
