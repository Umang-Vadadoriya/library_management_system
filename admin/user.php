...
$result_show_users = mysqli_query($conn, $query);
	...
	<div class="card mb-4">
		...
		<div class="card-body">
			<table id="datatablesSimple">
				...
				<tbody>
					<?php
					if (mysqli_num_rows($result_show_users) > 0) {
						foreach ($result_show_users as $row) {
							$user_status = '';
							if ($row['user_status'] == 'Enable') {
								$user_status = '<div class="badge bg-success">Enable</div>';
								$user_status_btn = '<button type="button" name="delete_button" class="btn btn-danger btn-sm" onclick="delete_data(`' . $row["user_id"] . '`, `' . $row["user_status"] . '`)">Disable</button>';
							} else {
								$user_status = '<div class="badge bg-danger">Disable</div>';
								$user_status_btn = '<button type="button" name="delete_button" class="btn btn-success btn-sm" onclick="delete_data(`' . $row["user_id"] . '`, `' . $row["user_status"] . '`)">Enable</button>';
							}
							echo '
    					<tr>
